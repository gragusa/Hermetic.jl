...
function get_inter_idx(X::Array{T, 2}, ki::T) where T<:Int
    ...
    rank = sum(X, dims=2)
    k = maximum(rank)
    nz = sum(map(x -> x == 0 ? 1 : 0, X), dims=2)
    ## sum(map(x -> x == 0 ? 1 : 0, X), dims=2) ## Number of zero in composition
    
    # Interactions are those allocation with more than 1 zero. Or, non 
    # interaction are those allocations with exactly M-1 zeros plus the case 
    # with M zeros
    idx = (nz .>= m-1)
    if ki == k
    ...
    elseif ki > 1 & ki < k
        ...
        idx = BitArray(( (one(T).-idx) .& (rank .== ki) ) .| idx)
    end
    return idx
end
...
Input, int E[O], the indices of the exponents of the polynomial.
...
"""
function Henp_to_polynomial(m::Int, l::Array{T, 1}) where T<:Int
    o1 = 1
    c1 = [1.0]
    e1 = Int[1]
...
struct Hermite <: PolyType end
struct Standard <: PolyType end
...
function convert(p::ProductPoly{H}) where {H<:Hermite}
    o, c, e = Henp_to_polynomial_fullw(p.m, p.o, p.c, p.e)
    ProductPoly(p.m, p.k, o, c, e, Standard())
end

function Base.:*(p::ProductPoly{S}, q::ProductPoly{S}; retain_zero::Bool = false) where S<:Standard
    @assert p.m == q.m
...
function Base.:+(p::ProductPoly{T}, q::ProductPoly{T}) where T<:Standard
    @assert p.m == q.m
    o, c, e = polynomial_add(p.o, p.c, p.e, q.o, q.c, q.e)
    ProductPoly(p.m, p.k + q.k, o, c, e, Standard())
end

function Base.:*(p::ProductPoly{H}, q::ProductPoly{S}) where {H<:Hermite, S<:Standard}
    p = convert(p)
    p * q
end

function Base.:*(x::Number, p::ProductPoly{S}) where {S<:Standard}
    c = copy(p.c)
    o, c, e = polynomial_scale(x, p.m, p.o, c, p.e)
    ProductPoly(p.m, p.k, o, c, e, Standard())
end

function Base.:*(x::Number, p::ProductPoly{H}) where {H<:Hermite}
    p = convert(p)
    c = copy(p.c)
    o, c, e = polynomial_scale(x, p.m, p.o, c, p.e)
    ProductPoly(p.m, p.k, o, c, e, Standard())
...
function Base.:*(p::ProductPoly{S}, q::ProductPoly{H}) where {H<:Hermite, S<:Standard}
    q = convert(q)
    p * q
end

function Base.:*(p::ProductPoly{H}, q::ProductPoly{H}) where {H<:Hermite}
    q = convert(q)
    p = convert(p)
    p * q
end

function Base.:^(p::ProductPoly{T}, j::Integer) where T
    if j == 1
        return p
    end
    for h in 2:j
...
function Base.broadcast(::typeof(*), s::Real, p::ProductPoly{T}) where T
    c = copy(p.c)
    o, c, e = polynomial_scale(s, p.m, p.o, c, p.e)
    ProductPoly(p.m, p.k, o, c, e, Standard())
end

function LinearAlgebra.rmul!(p::ProductPoly{S}, s::Real) where S
    polynomial_scale(s, p.m, p.o, p.c, p.e)
end

function polyval(p::ProductPoly{S}, x::Array{T, 2}) where {S<:Standard, T<:Real}
    polynomial_value(p.m, p.o, p.c, p.e, x)
end

function polyval(p::ProductPoly{S}, x::Array{T, 2}) where {S<:Hermite, T<:Real}
    polynomial_value(p.m, p.o, p.c, p.e, x)
end

integrate(p::ProductPoly{T}) where T<:Standard = integrate_polynomial(p.m, p.o, p.c, p.e)

function Base.show(io::IO, p::ProductPoly{T}) where T <: Standard
    println("ProductPoly{Standard} - Dimension: ", p.m, " - Order: ",
    p.k)
    polynomial_print(p.m, p.o, p.c, p.e; title = "P(z) = ")
end

function Base.show(io::IO, p::ProductPoly{T}) where T<:Hermite
